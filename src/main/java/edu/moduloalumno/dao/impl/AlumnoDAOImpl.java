...
public class AlumnoDAOImpl implements IAlumnoDAO {
	...
	public Alumno getAlumnoBycodigo(String codigo) {
		String sql = "SELECT id_alum, ape_nom, codigo, dni, id_facultad  FROM alumno WHERE codigo = ?";
		RowMapper<Alumno> rowMapper = new BeanPropertyRowMapper<Alumno>(Alumno.class);
		Alumno alumno = jdbcTemplate.queryForObject(sql, rowMapper, codigo);
		return alumno;
	}
	...
	public List<Alumno> getAlumnoIdByNombresApellidosRestringido(String nombresApellidos) {
		...
		"and not exists (select 1 from alumno_programa ap where ap.cod_alumno=r.cod_alumno)";
